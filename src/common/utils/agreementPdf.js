...
async function generateAgreementPDF(partner, signatureBase64, plainIdType = null, plainIdNumber = null) {
  const now = new Date();
  ...
  // Use provided plain ID or fallback to N/A
  const displayIdType = plainIdType || 'ID';
  const displayIdNumber = plainIdNumber || 'N/A';

  const html = buildAgreementHtml(partner, signatureBase64, LOGO_BASE64, agreementDate, agreementTime, displayIdType, displayIdNumber);
...
function buildAgreementHtml(partner, signatureBase64, logoBase64, agreementDate, agreementTime, displayIdType, displayIdNumber) {
  ...
  <div class="page">
    ...
    <div class="cover-wrapper">
      ...
      <div class="cover-details">
        ...
        <div class="detail-row"><strong>${escapeHtml(displayIdType)} Number:</strong> <span class="id-number-highlight">${escapeHtml(displayIdNumber)}</span></div>
    ...
    <div class="signature-wrapper">
      ...
      <div class="signature-block">
        ...
        <div class="signatory-info">
          ...
          Mobile: ${escapeHtml(partner.phone_number)}
        </div>
        
        <!-- ID Number displayed clearly near signature -->
        <div class="id-info-box">
          <div class="id-info-label">${escapeHtml(displayIdType)} Number</div>
          <div class="id-info-value">${escapeHtml(displayIdNumber)}</div>
        </div>
        
        <div class="signatory-info" style="margin-top: 2mm;">
          Agreement ID: ${escapeHtml(agreementId)}
        </div>
      </div>

      <div class="electronic-note">
        This agreement is electronically generated by Homecarehelp and does not require a physical signature of the company.<br>
        ID Verification: ${escapeHtml(displayIdType)} ${escapeHtml(displayIdNumber)}
      </div>
    </div>
  </div>
